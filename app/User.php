...
class User extends Authenticatable
{
    ...
    {
        if (!empty($request['refundable_user_id']) && !empty($request['payment_method'])) {
            $payment_gateway = !empty($request['payment_method']) ? $request['payment_method'] : 'Mpesa';
            $payment_settings = SiteManagement::getMetaValue('commision');
            $currency  = !empty($payment_settings) && !empty($payment_settings[0]['currency']) ? $payment_settings[0]['currency'] : 'KES';
            $user = User::find($request['refundable_user_id']);
            $payout = new Payout();
            $payout->user()->associate($request['refundable_user_id']);
            $payout->amount = $request['refundable_amount'];
            $payout->payment_method = $payment_gateway;
            $payout->currency = $currency;
            if (Schema::hasColumn('payouts', 'email')) {
                $payout->email = !empty($user->profile->payout_id) ? $user->profile->payout_id : 'empty';
            } elseif (Schema::hasColumn('payouts', 'paypal_id')) {
                $payout->paypal_id = !empty($user->profile->payout_id) ? $user->profile->payout_id : 'empty';
            }
