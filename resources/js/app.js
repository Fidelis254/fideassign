    ...
    const VueSettings = new Vue({
        ...
        methods: {
            ...
            importUpdate: function () {
                var self = this;
                this.$swal({
                    title: 'Import New Tables to your Database',
                    type: "warning",
                    customContainerClass: 'hire_popup',
                    showCancelButton: true,
                    confirmButtonClass: "btn-danger",
                    ...
                    if (result.value) {
                        axios.get(APP_URL + '/admin/import-updates')
                            .then(function (response) {
                                if (response.data.type == "success") {
                                    self.loading = false;
                                    self.$swal('Imported', 'Tables Imported Successfully', 'success')
                                } else {
                                    self.loading = false;
                                }
                            })
    ...
    const jobVue = new Vue({
        ...
        methods: {
            ...
            submitRefund: function () {
                this.loading = true;
                var self = this;
                var refundable_amount = $('#refundable-amount').val();
                var refundable_order = $('#refundable-order-id').val();
                var job_id = $('#refundable-job-id').val();
                let form = document.getElementById('submit_refund_' + job_id);
                let form_data = new FormData(form);
                form_data.append('refundable_amount', refundable_amount);
                form_data.append('refundable_user_id', self.refundable_user);
                form_data.append('order_id', refundable_order);
    ...
    const vservices = new Vue({
        ...
        methods: {
            ...
            submitRefund: function () {
                ...
                var self = this;
                var refundable_amount = $('#refundable-amount').val();
                var order_id = $('#refundable-order-id').val();
