														...
														@if ($order->status == 'cancelled' && Helper::getOrderPayout($order->id)->count() == 0)
															...
															<b-modal ref="myModalRef-{{ $order->id }}" hide-footer title="Refund" v-cloak>
																<div class="d-block text-center">
																	{!! Form::open(['url' => '', 'class' =>'wt-formtheme', 'id' => 'submit_refund_'.$order->id,  '@submit.prevent'=>'submitRefund']) !!}
																		<fieldset>
																			...
																			<input type="hidden" value="{{$service->price}}" id="refundable-amount" name="proposal_amount">
																			<input type="hidden" value="{{$order->id}}" id="refundable-order-id" name="order_id">
